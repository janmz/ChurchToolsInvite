...
func TestListPersonGroupsUsesGroupIDNotMembershipID(t *testing.T) {
	...
	if len(groups) != 1 {
		t.Fatalf("expected deduped group, got %+v", groups)
	}
	if groups[0].ID != 15 {
		t.Fatalf("group id = %d, want 15", groups[0].ID)
	}
}

func TestSortGroupsByName(t *testing.T) {
	groups := []churchtools.Group{
		{ID: 2, Name: "Zeta"},
		{ID: 1, Name: "Alpha"},
		{ID: 3, Name: "alpha"},
	}
	churchtools.SortGroupsByName(groups)
	if churchtools.PlainGroupName(groups[0].Name) != "Alpha" || groups[1].ID != 3 {
